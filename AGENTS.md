...
### Rule 7 — No free floating functions

> "Everything should be colocated with the struct or trait it is associated with. Unless it is specifically required to be a free floating function, it should be associated with a struct or trait."
...
```rs
pub fn match_host_url_patterns(patterns: HostUrlPatterns, paths: Vec<String>) -> MatchResult {
   // implementation
}
```
...
```rs
pub trait MatchHostUrlPatterns {
   fn match_host_url_patterns(&self, paths: Vec<String>) -> MatchResult;
}

impl MatchHostUrlPatterns for HostUrlPatterns {
   fn match_host_url_patterns(&self, paths: Vec<String>) -> MatchResult {
      // implementation
   }
}
