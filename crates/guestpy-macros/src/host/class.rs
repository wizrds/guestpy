...
impl ClassMember {
    ...
    fn context_types(&self) -> Vec<&Type> {
        match self {
            ...
            | Self::Getter(callable)
            | Self::Setter(callable)
            | Self::Deleter(callable) => callable.context_types(),
            Self::Statics(_) | Self::Constant { .. } => Vec::new(),
        }
    }
    ...
    fn classify_method(
        ...
        if callable.asynchronous() {
            return Err(syn::Error::new(
                callable.span(),
                r#"
async fn is unsupported in host classes;
for an async method use #[guestpy(async_method)] on a non-async fn returning
Result<impl core::future::Future<Output = Result<R, Error>> + use<..>, E>,
naming the backend type parameter inside use<..>
"#,
            )
            .into());
        }
...
"#,
    ...
    fn render(self, item: &ItemImpl) -> TokenStream {
        ...
        let mut context_types = constructor
            .iter()
            .flat_map(Callable::context_types)
            .collect::<Vec<_>>();
        ...
        let requirements = context_types
            .iter()
            .map(|ty| quote!(builder.require::<#ty>();));
        let construct = constructor.as_ref().map(|callable| {
            let ident = callable.ident();
            let enter = callable.enter_ident();
            let args = callable.args_ident();
            let bindings = callable.argument_bindings();
            let context = callable.context_setup(quote!(
                #crate_path::host::context::CallContext::class::<Self>(#enter)
            ));
            let setup = callable.argument_setup();
        ...
        let builder = if constructor.is_none()
            && context_types.is_empty()
            && members.is_empty()
            && extends.is_empty()
            && !generic
        {
...
mod tests {
    ...
    #[test]
    fn generates_one_requirement_for_repeated_class_context() {
        let output = expand(
            quote!(name = "Client", crate_path = crate),
            parse_quote! {
                impl Client {
                    #[guestpy(constructor)]
                    fn new(
        ...
        assert_eq!(
            output
                .matches("builder . require :: < ModuleState < HttpModule > > ()")
                .count(),
            1,
        );
        assert!(output.contains("CallContext :: class :: < Self > (__guestpy_enter)"));
        assert!(output.contains("__guestpy_context . resolve () ?"));
    }
    ...
    #[test]
    fn generates_handle_and_delete_roles_and_a_generic_hook() {
        let output = expand(
    ...
    #[test]
    fn selects_the_paired_registrar_for_a_shared_receiver_and_this() {
        let output = expand(
            quote!(name = "Ledger", backend = B, crate_path = crate),
            parse_quote! {
                impl Ledger {
                    #[guestpy(method)]
                    fn describe(&self, #[guestpy(this)] this: &Object<B>) -> Result<String, Error> {
                        this.type_name()
                    }
                }
            },
    ...
    #[test]
    fn selects_the_exclusive_paired_registrar_for_an_exclusive_receiver_and_this() {
        let output = expand(
            quote!(name = "Ledger", backend = B, crate_path = crate),
            parse_quote! {
                impl Ledger {
    ...
    #[test]
    fn selects_the_paired_async_registrar_for_a_shared_receiver_and_this() {
        let output = expand(
            quote!(name = "Ledger", backend = B, crate_path = crate),
            parse_quote! {
                impl Ledger {
    ...
    #[test]
    fn rejects_an_exclusive_receiver_on_an_async_method() {
        let Err(HostMacroError::Syntax(error)) = HostClassMacro::new(
            quote!(name = "Ledger", backend = B, crate_path = crate),
            parse_quote! {
                impl Ledger {
    ...
    #[test]
    fn rejects_a_method_with_neither_a_receiver_nor_this() {
        let Err(HostMacroError::Syntax(error)) = HostClassMacro::new(
            quote!(name = "Ledger", backend = B, crate_path = crate),
            parse_quote! {
                impl Ledger {
                    #[guestpy(method)]
                    fn describe() -> Result<String, Error> {
                        Ok(String::new())
                    }
                }
            },
    ...
    #[test]
    fn omits_constructor_registration_when_no_constructor() {
        let output = expand(
            quote!(name = "Session", crate_path = crate),
            parse_quote! {
                impl Session {
